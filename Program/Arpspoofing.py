    ...
    try:
        if system == 'windows':
            ...
            for line in out.splitlines():
                m = re.search(r'(\d+\.\d+\.\d+\.\d+)\s+((?:[0-9a-f]{2}[-:]){5}[0-9a-f]{2})', line, re.I)
                if m:
                    entries.append({'ip': m.group(1), 'mac': m.group(2).replace('-', ':')})
        else:
...
class MITMInterface(ctk.CTk):
    ...
    def validate_ip(self, ip):
        return re.match(r'^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$', ip) and all(0 <= int(x) <= 255 for x in ip.split('.'))
        ...
        try:
            while True:
                try:
                    ...
                    lines = float(self.log_text.index('end-1c').split('.')[0])
                    if lines > 200:
